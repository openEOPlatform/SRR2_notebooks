...
from openeo.processes import ProcessBuilder, array_modify, array_concat, array_apply, power, sqrt, clip, if_, multiply, divide, arccos
import openeo.processes as prc
import numpy as np
...
def _callback(x: ProcessBuilder, index_list:list, datacube:DataCube) -> ProcessBuilder:
	...
	for idx in index_list:
		...
		band_indices = [datacube.metadata.get_band_index(band) for band in indices[idx].__code__.co_varnames[:indices[idx].__code__.co_argcount]]
		lenx += 1
...
bbox = (4.73382, 50.795237, 4.734988, 50.795519)
...
#                                 temporal_extent=["2018-01-01", "2018-01-04"], 
#                                 bands=["B04","B08","B11"])
...
feats = compute_indices(s2_masked, ["ANIR","NDVI"])
